...
function startSyntaxExample(){
  ...
  if(steps.length > 0){
    ...
    if(infoEl) 
      infoEl.innerHTML = `${currentCommandIndex + 1}. ${s.description}`;
    if(detailEl)
      detailEl.innerHTML = (s.detail && s.detail.trim()) ? s.detail : (langData.detailNotProvided || `<span class="noDetail">${langData.noDetailProvided || ''}</span>`);
  }
}
...
function nextSyntaxStep(){
  ...
  if(currentCommandIndex < steps.length - 1){
    ...
    if(infoEl)
      infoEl.innerHTML = `${currentCommandIndex + 1}. ${s.description}`;
    if(detailEl)
      detailEl.innerHTML = (s.detail && s.detail.trim()) ? s.detail : `<span class="noDetail">${langData.noDetailProvided || langData.detailNotProvided || ''}</span>`;
  }
  else{
