...
function mpCommitManualStep(step){
  ...
  const prevTotal = mpManualHistory.length ? (mpManualHistory[mpManualHistory.length - 1].step?.totalAmortizedCost ?? 0) : 0;
  mpDecorateRecordedStep(step, prevTotal);
...
function mpRunAdditionAnimation(item){
  if(!item || mpSkipAnimations)
    return;

  const token = mpRunToken;
  mpBusy = true;
  mpSetMainButtonsEnabled(false);
  ...
  item.addEventListener('animationend', () => {
    ...
    mpBusy = false;
    mpSetMainButtonsEnabled(true);
    setSyntaxNextEnabled(true);

    if(document.getElementById('mpSynPanel'))
      mpSynRenderInfo();
    if(document.getElementById('mpScPanel'))
      mpScRenderInfo();
  }, {once: true});
}
...
function pushToStackManual(value){
  ...
  mpRunAdditionAnimation(newItem);
  updateCounters();
