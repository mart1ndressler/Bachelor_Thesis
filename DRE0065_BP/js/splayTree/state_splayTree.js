...
let sScSteps = [];
let sScIndex = 0;
let sScExecutionMode = 'auto';
let sScPaused = false;
let sScFinished = false;
let sScHistory = [];
let sScResults = [];
let sScAutoTimer = 0;
let sScHistoryCursor = -1;
let sScFastForward = false;
let sScInitialSteps = [];
let sSynSteps = [];
let sSynCursor = 0;
let sSynResults = [];
...
let sScPreviewVisible = false;
let sSynPreviewVisible = false;
let sSynPreviewHideTimer = 0;
...
function sBuildAnalysisBlock(step, result, L = sLang()){
  ...
  return `
    ...
    <ul class="ti-ul18">
      ...
      <li>${L.infoMeaningCurrentStepAmortizedCost || 'â<sub>k</sub> is the amortized cost of the current operation.'}</li>
...
function sBuildStepLabel(step, L = sLang()){
  const template = step.op === 'insert' ? (L.splayScStepInsertTitle || 'Insert {value}') : step.op === 'search' ? (L.splayScStepSearchTitle || 'Search {value}') : (L.splayScStepDeleteTitle || 'Delete {value}');
...
function sResetAll(){
  ...
  sRunToken += 1;
  sEdgeAnimToken += 1;
  sScPreviewVisible = false;
  sSynPreviewVisible = false;
