function sFormatRandomStep(step){
  return step ? sBuildStepLabel(step, sLang()) : '';
}

function sGetRandomPreviewState(){
  return {prev: sFormatRandomStep(sRandomSteps[sRandomCursor - 2]), current: sFormatRandomStep(sRandomSteps[sRandomCursor - 1]), next: sFormatRandomStep(sRandomSteps[sRandomCursor])};
}

function sRenderRandomPreview(animate = false){
  typesParamsUpdateOperationPreview('s', sGetRandomPreviewState(), animate);
}
...
function sRandomAdvanceAfterExecutedStep(){
  ...
  if(sRandomCursor >= sRandomSteps.length){
    ...
    sSynPreviewHideTimer = setTimeout(() => {
      if(!sIsRandomMode)
        return;
