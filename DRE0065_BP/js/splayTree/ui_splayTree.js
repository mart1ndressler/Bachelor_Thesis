...
function sBwRenderInfo(){
  ...
  const modeDesc = (sBwExecutionMode === 'auto') ? (Lall.bwAutoPanelDesc || '') : (Lall.bwManualPanelDesc || '');
  ...
  if(sBwHistory.length === 0){
    const preview = sBwBuildFirstPreview();
...
function sSynUpdateFinishUI(){
  ...
  forwardBtn.disabled = sSynHistoryCursor >= sSynSteps.length - 1;
}
...
function refreshSplayUIForLanguage(){
  ...
  if(document.getElementById('sBwPanelTitle') && (!Array.isArray(sBwHistory) || sBwHistory.length === 0))
  sBwRenderInfo();
