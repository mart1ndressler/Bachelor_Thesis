(() => {
  ...
  const k = {n: 150, d: 150, r: 7.2, rs: 2.0, lw: 1.5, la: 0.18, v: 0.32, hr: 220, hs: 2.6, pr: 3.6, pa: 0.55, ps: 0.16, pn: 7, pm: 420, bs: 1.25, ba: 0.10};
  ...
  function R(p, q){
    return p + Math.random() * (q - p);
  }

  function C(v, p, q){
    return Math.max(p, Math.min(q, v));
  }
  ...
  function fit(){
    ...
    if(!a.length){
      a = Array.from({length: k.n}, () => ({x: R(0, w), y: R(0, h), vx: R(-k.v, k.v), vy: R(-k.v, k.v), ph: R(0, Math.PI * 2)}));
      return;
    }
  ...
  window.addEventListener("mouseleave", () => {m.on = false; m.x = -1e9; m.y = -1e9; m.lx = -1e9; m.ly = -1e9;}, {passive: true});
