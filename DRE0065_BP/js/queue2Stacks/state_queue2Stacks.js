...
let qIsSpecialCasesMode = false;
let qSpecialCasesMode = null;
let qRunTimer = null;
let qSynHistory = [];
...
let qScSteps = [];
let qScCursor = 0;
let qScExecutionMode = 'auto';
let qScPaused = false;
let qScFinished = false;
let qScHistory = [];
let qScHistoryCursor = -1;
let qScFastForward = false;
let qScInitialScenario = null;
...
let qRandomSteps = [];
let qRandomCursor = 0;
let qScPreviewVisible = false;
let qSynPreviewVisible = false;
let qSynPreviewHideTimer = 0;
let qRunToken = 0;
let qSynPendingPreview = null;
...
function qShouldTrackManualHistory(){
  return !qIsSpecialCasesMode && !qIsSyntaxMode && !document.getElementById('qSynPanel');
}

function qGetOperationCount(){
  if(qIsSpecialCasesMode){
    if(qScHistoryCursor >= 0)
      return qScHistoryCursor + 1;
...
function qClearSyntaxPreviewHideTimer(){
  if(qSynPreviewHideTimer){
    clearTimeout(qSynPreviewHideTimer);
    qSynPreviewHideTimer = 0;
  }
}
...
function qResetSyntaxRuntimeState(){
  ...
  qSynHistoryCursor = -1;
  qSynFinished = false;
  qSynPreviewVisible = false;
  qSynPendingPreview = null;
}

function qResetSpecialCasesRuntimeState(){
  qIsSpecialCasesMode = false;
  qSpecialCasesMode = null;
  qScCommands = [];
  qScSteps = [];
  qScCursor = 0;
  qScExecutionMode = 'auto';
  qScPaused = false;
  qScFinished = false;
  qScHistory = [];
  qScHistoryCursor = -1;
  qScFastForward = false;
  qScInitialScenario = null;
  qScPreviewVisible = false;
}
...
function qResetState(){
  ...
  qResetSpecialCasesRuntimeState();
