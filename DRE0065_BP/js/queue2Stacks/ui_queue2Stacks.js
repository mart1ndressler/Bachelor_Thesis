        ...
        <div class="run-side-info">
          <div class="syntax-detailed-info" id="qBwPanel">
            <div class="syntax-detailed-scroll">
              <div class="info-panel-title" id="qBwPanelTitle">${scenario.title}</div>
...
function qBwRenderInfo(){
  ...
  const modeDesc = (qBwExecutionMode === 'auto') ? (Lall.bwAutoPanelDesc || '') : (Lall.bwManualPanelDesc || '');
  ...
  if(qBwHistory.length === 0){
    const previewStep = qBwBuildPreviewStep(qBwSteps[0]);
...
function qSynUpdateFinishUI(){
  ...
  forwardBtn.disabled = qSynHistoryCursor >= qSyntaxSteps.length - 1;
}
