...
function qExecuteSyntaxStep(step, done){
  ...
  dequeueQueue2Stacks((removed) => {
    if(removed === undefined){
      ...
      const tpl = L.emptyDequeueDetail || `<span class="emptyStackMessage">${L.queueEmptyNoDequeue}</span>`;
      step.detail = qFmt(tpl, {potential: `<span class="potentialValue">${qPotential}</span>`});
...
function rebuildQueueSyntaxStepsForLanguage(){
  ...
  for(const s of newSteps){
    ...
    if(s.type === 'dequeue'){
      if(s.wasEmpty){
        const tpl = L.emptyDequeueDetail || `<span class="emptyStackMessage">${L.queueEmptyNoDequeue}</span>`;
        s.detail = qFmt(tpl, {potential: `<span class="potentialValue">${qPotential}</span>`});
      }
