...
function refreshCurrentViewAfterLanguageChange(){
  if(currentPage === 'multipop'){
    if(document.getElementById('mpBwPanel')){
  ...
  if(currentPage === 'queue2Stacks'){
    if(document.getElementById('qBwPanel')){
    ...
    if(typeof qBusy !== 'undefined' && qBusy){
      ...
      const enqueueBtn = document.getElementById('qEnqueueBtn');
      const dequeueBtn = document.getElementById('qDequeueBtn');
...
function changeLanguage(language){
  ...
  const manualExampleEl = document.getElementById('manualParamsExample');
  if(manualExampleEl){
    const isQueueManual = (activeModalContext === 'queueManual');
  ...
  const randomSection = activeModalContext === 'queueRandom' ? 'queue' : activeModalContext === 'splayRandom' ? 'splay' : 'multipop';

  const Lrandom = trPage(randomSection, language);

  const randomInfoEl = document.getElementById('randomParamsInfo');
  if(randomInfoEl)
    randomInfoEl.innerHTML = Lrandom.randomParamsInfo || '';

  const rangeMinExampleEl = document.getElementById('rangeMinExample');
  if(rangeMinExampleEl)
    rangeMinExampleEl.innerHTML = Lrandom.rangeMinExample || '';

  const rangeMaxExampleEl = document.getElementById('rangeMaxExample');
  if(rangeMaxExampleEl)
    rangeMaxExampleEl.innerHTML = Lrandom.rangeMaxExample || '';

  const countExampleEl = document.getElementById('countExample');
  if(countExampleEl)
  ...
  if(document.getElementById('syntaxModalLabel')){
    ...
    if(synInput)
        synInput.removeAttribute('placeholder');
