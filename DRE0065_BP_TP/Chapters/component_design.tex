...
Pro lepší přehled o navržené struktuře celé aplikace byl uveden také UML komponentový diagram, který zachycuje hlavní části implementované komponenty a vztahy mezi nimi. Hlavním účelem tohoto diagramu je především to, aby bylo jasné, jak jsou jednotlivé soubory a logické celky aplikace rozděleny, jaké jsou jejich role a jak mezi sebou navzájem spolupracují.
...
Na obrázku \ref{fig:component_diagram} je zachycena výsledná architektura aplikace v podobě UML komponentového diagramu. Diagram shrnuje hlavní části implementace a jejich vazby, přičemž názorně ukazuje oddělení sdílených prostředků, centrální aplikační logiky, zpracování vstupních režimů a samostatných algoritmů.
